...
weights2 = [-0.4213494781970466,-1.289752632014824,2.131147137055913,3.0013040025302273,0.7776877552461124,-4.9130609416025965,0.33218474305050627,-0.9623169375270323,2.182771260380689,3.398312751722844];

biases1 = [0.006645937727594428,0.2704243518622918,1.6027427386847641,-0.830943744533214,-2.3572969492183304,-0.5222825618402926,-0.6020729386028985,-1.5685247612411921,-1.7327961861427545,-0.5496768945296635];
biases2 = [1.2425585686811251];

layer1 = zeros(nodesCount(1), 1);
layer2 = zeros(nodesCount(2), 1);
layer3 = zeros(nodesCount(3), 1);
...
%% ver1
for ii = 1:nodesCount(2)
    temp = 0;
    for jj = 1:nodesCount(1)  
        temp = temp + layer1(jj) * weights1(ii, jj);
    end
    layer2(ii) = temp + biases1(ii);
    % ReLU activation
    for nn = 1:length(layer2)
        if layer2(nn) < 0
            layer2(nn) = 0;
        end
    end
...
for ii = 1:nodesCount(3)
    temp = 0;
    for jj = 1:nodesCount(2)  
        temp = temp + layer2(jj) * weights2(ii, jj);
    end
    layer3(ii) = temp + biases2(ii);
    % logistic actication
    for nn = 1:length(layer3)
        layer3(nn) = 1 / (1 + exp(-layer3(nn)));
    end
end
